...
export default function SmartCarousel({
  ...
  const carouselItems: UIItem[] = useMemo(() => {
    const list = caseStudies.map((caseStudy, i) => {
      const image = assetUrl(caseStudy.mainImage) || "";
      const logosrc = assetUrl(caseStudy.logoImage) || undefined;
      const video = assetUrl(caseStudy.mainVideo) || undefined;
      const linkHref = caseStudy.slug?.current
        ? `/${language}/cases/${caseStudy.slug.current}`
        : undefined;
      ...
      return {
        ...
        subtitle:
          caseStudy.subtitle ||
          (caseStudy.category ? caseStudy.category.join(", ") : ""),
        image,
        video,
        description: caseStudy.description || "",
        category: caseStudy.category
          ? caseStudy.category.join(", ")
          : undefined,
        logosrc,
        linkHref,
      } as UIItem;
        ...
        <div className="container mx-auto w-full">
          <div className="relative h-[800px] flex items-center justify-center">
            <div className="text-gray-400">
              No case studies found for {channelName} Smart Carousel
            </div>
          </div>
        </div>
      </section>
