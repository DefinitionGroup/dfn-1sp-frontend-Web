...
type StrandsSettings = Required<Omit<StrandsProps, "className" | "style">> & {
  palette: number[][];
  colorCount: number;
};

type MutableProgram = Program & {
...
export default function Strands({
  ...
  const propsRef = useRef<StrandsSettings>({
    active,
    colors,
    count,
    speed,
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColors: { value: initial.palette },
        uColorCount: { value: initial.colorCount },
    ...
    const update = (time: number) => {
      if (!isVisible || !propsRef.current.active) {
        animationFrame = 0;
        return;
      }
      ...
      program.uniforms.uColors.value = current.palette;
      program.uniforms.uColorCount.value = current.colorCount;
    ...
    const startLoop = () => {
      if (isVisible && propsRef.current.active && animationFrame === 0) {
        animationFrame = requestAnimationFrame(update);
      }
    };
