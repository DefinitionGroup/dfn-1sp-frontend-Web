...
const fragmentShaderSource = `
  ...
  void main() {
    ...
    // Define two tonal groups within the Renaissance teal family.
    ...
    // Calculate primary teal metaballs
    tealField += metaball(p, ball1, size1);
    tealField += metaball(p, ball2, size2);
    tealField += metaball(p, ball3, size3);

    // Calculate lighter mist metaballs
    mistField += metaball(p, ball4, size4);
    mistField += metaball(p, ball5, size5);
    mistField += metaball(p, ball6, size6);

    // Total field for threshold
    field = tealField * mistField;

    // Colors
    vec3 tealColor = vec3(0.0, 0.553, 0.655);
    vec3 mistColor = vec3(0.271, 0.710, 0.788);
    vec3 bgColor = vec3(0.949, 0.980, 0.984);
    ...
    float totalField = tealField + mistField + 0.001;
    float tealRatio = tealField / totalField;
    float mistRatio = mistField / totalField;

    // Mix colors based on field dominance
    vec3 ballColor = tealColor * tealRatio + mistColor * mistRatio;
