...
interface MixedTypeProps {
  text: string;
  className?: string; // Base class for the Sans text (e.g., font-renaissance)
  serifClassName?: string; // Legacy override class for *emphasized* text
}

export default function MixedType({
  text,
  className,
  serifClassName = "font-renaissance italic",
}: MixedTypeProps) {
  if (!text) return null;
