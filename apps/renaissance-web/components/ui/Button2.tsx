...
void main() {
  ...
  vec3 base = mix(TEAL, INK, u_dark);
  vec3 col = base;

  /* faint halo under the pointer so sparkles have a stage */
  float d = distance(p, m);
  col = mix(col, MIST, exp(-d * d * 6.0) * u_hover * 0.18);
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      ...
      col += mix(MIST, WHITE, tw) * glint * (1.6 + u_press * 1.2) * u_hover;
    }
  }
...
function Button2({
  ...
  const content = (
    ...
    >
      ...
      <span
        className={cn(
          "font-renaissance relative z-10 flex flex-col items-start justify-center whitespace-nowrap leading-none transition-colors duration-200",
