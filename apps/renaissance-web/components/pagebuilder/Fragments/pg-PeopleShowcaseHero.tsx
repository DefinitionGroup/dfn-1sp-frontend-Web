...
/* renaissance entrance system: decelerating bezier, 0.65s, 0.07s stagger.
   Cards slide up first; each card's frosted panel follows 0.2s later. */
const EASE_RENAISSANCE = [0.62, 0.05, 0.01, 0.99] as const;
...
const cardVariants = {
  ...
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.65, ease: EASE_RENAISSANCE },
  },
};
...
const panelItemVariants = {
  ...
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.45, ease: EASE_RENAISSANCE },
  },
};
          ...
          {visibleMembers.map((member, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative aspect-square flex-shrink-0 overflow-hidden rounded-card bg-neutral-100"
                data-member={(
                  member.name ||
                  member.fullname ||
                  ""
              ...
              >
                ...
                <motion.div
                  ...
                  className="absolute bottom-3 left-3 z-10 flex w-fit max-w-[calc(100%-1.5rem)] flex-col gap-0.5 rounded-card bg-[rgba(111,111,111,0.4)] px-4 py-3 backdrop-blur-md sm:px-5 sm:py-4"
                >
                  {hasVisibleText(member.fullname || member.name) && (
                    <motion.h3
                      variants={panelItemVariants}
                      className="text-white font-renaissance font-semibold text-sm sm:text-base leading-tight"
                    >
                    ...
                    >
                      {member.email && (
                        <a
                          ...
                          className="flex h-8 w-8 items-center justify-center rounded-control bg-white/90 text-[#245e66] backdrop-blur-sm transition-colors hover:bg-[#245e66] hover:text-white"
                        >
                          <svg
                            className="h-4 w-4"
                            fill="currentColor"
