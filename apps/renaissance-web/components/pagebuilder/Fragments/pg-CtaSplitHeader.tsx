...
function CtaSplitHeader({ data }: { data: CtaSplitHeaderType }) {
  ...
  return (
    <section className="grid grid-cols-12 container mx-auto gap-8 font-renaissance">
      {/* Left: CTA Mini */}
      {cta && (
          ...
          <div className="flex flex-col lg:gap-8 items-start justify-start w-full">
            <div className="flex-1 flex flex-col min-w-0">
              {hasVisibleText(data.heading) && (
                <h2 className="text-3xl text-neutral-900 font-renaissance">
                  {data.heading}
                </h2>
              )}
              {data.subheading && (
                <h4 className="text-3xl text-neutral-900 font-renaissance">
      ...
      {hasVisibleText(data?.paragraph) && (
        <header className="col-span-6 md:col-start-8">
          <h2 className="text-base text-neutral-900 font-renaissance">
            {data.paragraph}
          </h2>
        </header>
      )}
