...
function UnitLogoGrid({
  ...
  const getLinkHref = (unit: Unit): string | null => {
    ...
    return localizedPath(cta.href, language);
  };
          ...
          {unitsWithLogo.length === 0 ? (
            ...
            >
              {unitsWithLogo.map((unit, index) => {
                ...
                const logoContent = (
                    <div className="relative w-full aspect-[3/2] flex-col border border-neutral-200/90 items-center justify-center overflow-hidden">
                      <Image
                        src={logoUrl}
                ...
                const className = `group flex items-center justify-center hover:bg-neutral-200/20 p-2 md:p-4 ${bgClass} transition-all duration-300${href ? " cursor-pointer" : ""}`;
