...
export default function ListStep({ step }: { step: GalleryListStep }) {
  ...
  const applyLocaleToPath = (url?: string | null) => localizedPath(url, locale);
            ...
            <header className="col-span-12 px-4 md:px-0 md:col-span-6 col-start-1 iphone-landscape:!col-start-1 iphone-landscape:!col-span-12 md:mt-0">
              <div className="flex flex-col items-start justify-start w-full">
                <div className="flex-1 flex flex-col min-w-0">
                  {header?.superText && (
                    <h4 className=" text-neutral-900 font-bold font-renaissance">
                      {header.superText}
                    </h4>
                  )}
                  {hasVisibleText(header?.mainHeadline) && (
                    <h2 className="text-3xl md:text-7xl iphone-landscape:text-5xl text-gray-900 font-renaissance">
                      {header.mainHeadline}
                    </h2>
                  )}
                  {header?.subHeadline && (
                    <h4 className=" text-gray-700 font-medium mt-4  font-renaissance">
                      {header.subHeadline}
                    </h4>
                  )}
                </div>
