...
const PercentageDiagramVertical: React.FC<PercentageDiagramVerticalProps> = ({
  ...
  useEffect(() => {
    if (totalLength > 0 && isInView) {
      animate(currentLength, percentValue * totalLength, {
        duration: 1,
        ease: "easeOut",
      });
    }
  }, [percentValue, totalLength, isInView, currentLength]);
  ...
  return (
    <div ref={ref} className="relative w-[238px] h-[228px]">
      ...
      <motion.p
        className="bg-violet-500 px-2 w-fit  text-black -ml-2 text-xxs absolute"
