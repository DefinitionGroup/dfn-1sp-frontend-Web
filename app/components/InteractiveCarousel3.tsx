...
export default function InteractiveCarousel2() {
  ...
  return (
    <section className="  ">
      ...
      >
            ...
            <AnimatePresence initial={false} custom={direction}>
              ...
              >
                <div className="relative w-full h-full overflow-hidden bg-gradient-to-brshadow-2xl">
                  ...
                  {/* Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/0 border-red-500 to-transparent" />
        ...
        <div className="absolute  w-full   bottom-[16px] z-30 ">
          <div className="flex   justify-center mt-8 mx-auto space-x-2 bg-gray-900/50 backdrop-blur-xl h-10 items-center px-8 rounded-full w-fit">
            {carouselItems.map((_, index) => (
              <motion.button
                key={index}
                className={` h-2 rounded-full  transition-all hover:bg-lime-400 duration-300 cursor-pointer ${
                  index === currentIndex
        ...
        {/* Thumbnail Strip */}{" "}
        <div className="  flex justify-center mb-32  w-full  bottom-[200px] z-30   ">
          <div
            ref={stripRef}
            className={`flex absolute justify-center mt-4  space-x-4  pb-4 ${isScrollable ? "overflow-x-auto" : ""}`}
          >
            {carouselItems.map((item, index) => (
              <motion.button
                key={item.id}
                className={`flex-shrink-0 w-28 h-28  rounded-sm overflow-hidden outline-3 transition-colors ${
                  index === currentIndex
                    ? "outline-lime-500"
                    : "outline-transparent"
                }`}
